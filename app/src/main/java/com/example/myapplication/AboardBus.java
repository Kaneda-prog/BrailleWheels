...
public class AboardBus extends AppCompatActivity implements LocationListener,
    ...
    public class UpdateBusInfo extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void result) {
            ...
            if(pPosition.distanceTo(busStop)<400)
            {
                Toast.makeText(getApplicationContext(),"NICE", Toast.LENGTH_LONG).show();
                tts.setLanguage(Locale.forLanguageTag("pt"));
                tts.speak("Você está perto do ponto. Faltam " + dis,TextToSpeech.QUEUE_ADD, null);
    ...
    {
        tts.setLanguage(Locale.forLanguageTag("pt"));
        tts.speak("Você está perto do ponto",TextToSpeech.QUEUE_ADD, null);
